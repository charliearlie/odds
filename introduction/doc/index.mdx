...
All tokens are declared in dedicated `_*.scss` files in their dedicated tokens' components. They declare CSS Custom Properties, prefixed with `--fpsm-*`. They're then all exposed by the `all/all.scss` component global stylesheet.
