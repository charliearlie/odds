...
All tokens are declared in dedicated `_*.scss` files in their dedicated tokens' components. They declare CSS Custom Properties, prefixed with `--fpsm-*`. They're then all exposed by the `styles/index.scss` component global stylesheet.
...
```scss
.button {
  border-radius: var(--fpsm-border-radius-medium);
  background: var(--fpsm-color-primary);
  color: var(--fpsm-color-white);
}
```
